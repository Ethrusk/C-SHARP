...
// Для решении задачи используется функция diff, которая на выходе и получает разницу между максимальным 
...
Random rand = new Random(); 
        ...
        for (int i = 0; i < R; i++)
        {
            randomArray[i] = rand.Next(-100, 100) + rand.NextDouble(); // array[i] = new Random().Next(1, 101); 
            Console.Write($"{randomArray[i]:F2} "); // F2 - описание формата вывода, 
                                                    // где F - формат c плавающей запятой
                                                    // 2 - кол-во знаков после запятой
                                                    // rand.Next(-100, 100) - с любым диапазоном, если мы хотим 
                                                    // иметь какие-то цифры перед запятой, так как rand.NextDouble()
                                                    // выдаёт значения типа 0,02; 0,35 и т.п.
        }
}
...
// https://www.cyberforum.ru/csharp-beginners/thread1006929.html
